function K = get_k_length(leg_length)
    ...
    % Q=diag([80 1 150 20 3200 1]);%theta d_theta x d_x phi d_phi
    % R=diag([85,18]);              %T Tp
    % Q=diag([600 10 100 10 25000 30]);%theta d_theta x d_x phi d_phi
    % R=diag([10,1]);              %T Tp

    
    Q=diag([80 1 1000 500 12000 1]);%theta d_theta x d_x phi d_phi
    R=diag([50,12]);              %T Tp
